...
def get_outcome_states(state: int, action: str) -> List[int]:
    ...
    """
    assert isinstance(
        state, int) and 0 <= state < 16, "States must be an integer between 0 - 15."
...
def get_transition_probability(state: int, action: str, outcome_state: int) -> float:
    ...
    assert action in moves.keys(), "Action must be either: left, down, right, or up."
    assert isinstance(
        outcome_state, int) and 0 <= outcome_state < 16, "States must be an integer between 0 - 15."
...
def value_iteration() -> Any:
    ...
    number_actions = constants.number_actions
    epsilon = constants.epsilon
    U = []
    for i in range(0, number_states):
        U.append(0)

    # get_transition_probability (state: int, action: str, outcome_state: int) -> float:
    # get_reward(state int) -> float
    # get_outcome_states(state: int, action: str) -> List[int]:
    while True:
        delta = 0
        ...
        for s in range(0, number_states):
            ...
            U[s] = get_reward(s) + max([gamma*get_transition_probability(s, action, states)*U[states]
                                        for action in actions for states in get_outcome_states(s, action)])
            delta = max(delta, np.abs(U_old - U[s]))
    ...
    print(U[0], '   | ', U[1],  '            | ',
          U[2],  '             | ', U[3])
    print('-------------------------------------------------------------------------------------')
    print(U[4], '          | ', U[5],  '     | ', U[6],  '    | ', U[7])
    print('-------------------------------------------------------------------------------------')
    print(U[8], '            | ', U[9],  '     | ', U[10], '     | ', U[11])
    print('-------------------------------------------------------------------------------------')
    print(U[12], '   | ', U[13], '      | ', U[14], '     | ', U[15])
    print('\n')
    return U


def extract_policy(U) -> Any:
    """
    ...
    for i in range(0, constants.number_states):
        policy.append("left")
...
#        policy.append("left")
    ...
    for state in range(0, constants.number_states):
        for action in actions:
            current = helping_func(state, policy[state], U)
            new = helping_func(state, action, U)

            if(new > current):
                policy[state] = action
    for i in range(0, constants.number_states):
        print("state :", i+1, policy[i])

    # TODO: Implement the method.


def helping_func(state, action, U):
    return sum([get_transition_probability(state, action, states)*U[states] for states in get_outcome_states(state, action)])
...
'''
            if sum([get_transition_probability(state, action, states)*U[states]
                    for states in get_outcome_states(state, action)]) > sum([get_transition_probability(
                        state, policy[state], states)*U[states] for states in get_outcome_states(state, policy[state])]):
               policy[state] = action
'''
